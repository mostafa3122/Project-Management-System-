...
function generatePassword(lower: boolean, upper: boolean, number: boolean, symbol: boolean, length: number): string {
  const types = [lower && "lower", upper && "upper", number && "number", symbol && "symbol"].filter(Boolean) as string[];
  if (!types.length) return "";
  let result = "";
  for (let i = 0; i < length; i += types.length) types.forEach((t) => { result += randomFunc[t](); });
  return result.slice(0, length);
}

// ── Progress Stepper ──────────────────────────────────────────────────────────
function ProgressStepper({ currentStep, steps }: { currentStep: number; steps: { label: string }[] }) {
          ...
          return (
            <div key={step.label} className="relative z-10 flex flex-col items-center gap-1.5">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold border-2 transition-all duration-500
                ${isCompleted ? "bg-[#EF9B28] border-[#EF9B28] text-white"
                  : isActive  ? "bg-[#315951] border-[#EF9B28] text-[#EF9B28]"
              ...
              <span className={`text-[11px] font-medium tracking-wide transition-colors duration-300
                ${isActive ? "text-[#EF9B28]" : isCompleted ? "text-white/60" : "text-white/25"}`}>
                {step.label}
              </span>
            </div>
          );
...
function PasswordGeneratorPanel({ onUse, onClose }: { onUse: (p: string) => void; onClose: () => void }) {
  ...
  const handleGenerate = () => { setGenerated(generatePassword(lower, upper, numbers, symbols, length)); setCopied(false); };
  const handleCopy = () => {
    if (!generated) return;
  ...
  const handleUse = () => {
    if (!generated) return;
    onUse(generated); onClose(); toast.success("Password applied!");
  };
  ...
  return (
    <div className="fixed top-1/2 right-6 -translate-y-1/2 z-[9999] w-[300px] bg-[#1e3a35] border border-white/10 rounded-2xl p-5 shadow-[0_8px_32px_rgba(0,0,0,0.45)] text-white">
      <div className="flex justify-between items-center mb-4">
        <span className="font-semibold text-[15px] tracking-tight">Password Generator</span>
        <button onClick={onClose} className="bg-white/10 border-none rounded-lg text-white cursor-pointer px-2 py-1 text-base leading-none hover:bg-white/20 transition-colors" aria-label="Close">✕</button>
      </div>
      <div className="bg-black/35 rounded-xl px-3 py-2.5 mb-4 flex items-center gap-2 min-h-[44px]">
        <span className={`flex-1 font-mono text-[13px] break-all ${generated ? "text-[#a3d9c8]" : "text-white/30"}`}>{generated || "Click Generate…"}</span>
        <button onClick={handleCopy} title="Copy" className={`bg-transparent border-none flex-shrink-0 px-1 py-0.5 text-base transition-colors ${generated ? "cursor-pointer" : "cursor-default"} ${copied ? "text-[#EF9B28]" : "text-white/55"}`}>{copied ? "✓" : "⧉"}</button>
      </div>
      ...
      {[
        ...
        <label key={label} className="flex justify-between items-center text-[13px] text-white/75 mb-2.5 cursor-pointer">
          {label}<input type="checkbox" checked={value} onChange={(e) => set(e.target.checked)} className="accent-[#EF9B28] w-4 h-4" />
        </label>
      ))}
      <div className="flex gap-2 mt-1">
        <button onClick={handleGenerate} className="flex-1 py-2.5 bg-[#315951] border border-white/15 rounded-xl text-white text-[13px] font-semibold cursor-pointer hover:bg-[#3d6e63] transition-colors">Generate</button>
        <button onClick={handleUse} disabled={!generated} className={`flex-1 py-2.5 border-none rounded-xl text-white text-[13px] font-semibold transition-colors ${generated ? "bg-[#EF9B28] cursor-pointer hover:bg-[#d98820]" : "bg-[#EF9B28]/30 cursor-default"}`}>Use Password</button>
      </div>
    </div>
  );
}
...
useEffect(() => {
  ...
  return (
    <div className="h-fit register-bg flex justify-center items-center">
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="profile-img md:mx-auto bg-amber-50 rounded-full w-20 h-20 md:w-24 md:h-24 mb-4">
              <div className="bg-[#3159518A] w-full h-full rounded-full flex justify-center items-center">
                <input type="file" id="fileInput" className="hidden" {...register("profileImage")} accept="image/*" />
                <label htmlFor="fileInput" className="flex flex-col items-center gap-2 cursor-pointer py-4">
            ...
            <div className="flex flex-wrap gap-x-9 gap-y-3 mb-4">
              {registerInputs.map((input) => (
                <InputField
                  ...
                  type={input.type}
                  register={register}
                  error={errors[input.name as keyof RegisterFormData]?.message as string}
            ...
            <div className="flex justify-end mb-8">
              <button
                ...
                className={`flex items-center gap-1.5 border border-[#EF9B28]/50 rounded-lg text-[13px] font-semibold px-3.5 py-[7px] cursor-pointer tracking-wide transition-all duration-200 ${
                  showGenerator ? "bg-[#EF9B28] text-white" : "bg-[#EF9B28]/15 text-[#EF9B28]"
                }`}
              >
      ...
      {showGenerator && (
        <PasswordGeneratorPanel onUse={handleUsePassword} onClose={() => setShowGenerator(false)} />
      )}
    </div>
  );
}