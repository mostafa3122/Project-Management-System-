    ...
    const loadFormData = async () => {
      try {
        ...
        if (isEditMode && id) {
          ...
          if (task.employee && !employees.some((e: any) => e.id === task.employee.id)) {
            employees = [...employees, task.employee];
          }
         
          if (task.project && !projects.some((p: any) => p.id === task.project.id)) {
  ...
  return (
    <>
      ...
      <div className="bg-[#F8F9FB] py-4 px-6 min-h-[450px] flex items-center justify-center">
        {loading ? (
          <div className="bg-white rounded-2xl border border-[#F1F5F9] shadow-[0px_2px_4px_0px_#00000040] p-12 max-w-4xl w-full mx-auto flex flex-col items-center justify-center gap-3">
            <div className="w-12 h-12 border-4 border-[#315951]/20 border-t-[#315951] rounded-full animate-spin"></div>
            <p className="text-[#315951] font-montserrat font-medium text-sm">Loading task details...</p>
          </div>
        ) : (
          <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-2xl border border-[#F1F5F9] shadow-[0px_2px_4px_0px_#00000040] p-6 max-w-4xl w-full mx-auto font-quicksand">
